...
(function($)
{	
	var defaultHandlers = ['swfupload_loaded_handler','file_queued_handler','file_queue_error_handler','file_dialog_start_handler','file_dialog_complete_handler','upload_start_handler', 'upload_resize_start_handler', 'upload_progress_handler','upload_error_handler','upload_success_handler','upload_complete_handler','queue_complete_handler'];
	...
	$.fn.swfupload = function(){
		...
		return this.each(function(){
			...
			if (args.length == 1 && typeof(args[0]) == 'object') {
				...
				if (!swfu) {
					$(this).data('__swfu_back', $(this).clone());
					var settings = args[0];
	...
	$.swfupload = {
		...
		destroy: function(el) {
			$(el).data('__swfu').destroy();
			$(el).html($(el).data('__swfu_back').html());
			$(el).removeData('__swfu');
		}
	};
