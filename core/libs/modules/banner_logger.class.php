...
class Banner_Logger extends Singleton
{
	...
	{
		if (is_array($banners) && ($result = Model::$_db->query("SELECT `banner_id`, SUM(shows) as 'shows', SUM(clicks) as 'clicks' FROM `md_banners` WHERE (`date` BETWEEN '$date1' AND '$date2') AND `banner_id` IN(".join(',', $banners).") GROUP BY `banner_id` ORDER BY `shows` DESC")))
		{
			$records = array();
